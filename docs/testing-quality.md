...
`RoomTitleSelectionTest`는 제목만 읽는 조건과 실패 진단을 검증합니다. 루트·채팅 목록·입력창·툴바·제목의 누락/중복/숨김/다른 패키지, 오래된 노드와 빈 제목을 구분하며 실패 시 본문 텍스트를 읽지 않는지 검사합니다. 이 JVM 결과와 Maestro의 권한 안내·취소 확인은 실제 카카오톡 제목 선택 성공을 대신하지 않습니다. 실기기에서는 사용자가 현재 방 선택을 누른 뒤 `RoomSelection`의 고정 원인 코드와 앱의 제목 확인·등록을 확인합니다. 선택 세션 도중 다른 접근성 서비스를 억제하는 UiAutomation 덤프는 사용하지 않습니다.
