...
`RoomTitleSelectionTest`는 제목만 읽는 조건과 실패 진단을 검증합니다. 루트·채팅 목록·입력창·툴바·제목의 누락/중복/숨김/다른 패키지, 오래된 노드와 빈 제목을 구분하며 실패 시 본문 텍스트나 설명을 읽지 않는지 검사합니다. 제목 노드의 텍스트가 비었을 때만 같은 노드의 접근성 설명을 사용하고, 텍스트 우선순위·빈 값·512자 경계·길이 초과 시 대체 금지를 검사합니다. 이 JVM 결과와 Maestro의 권한 안내·취소 확인은 실제 카카오톡 제목 선택 성공을 대신하지 않습니다. 실기기에서는 사용자가 현재 방 선택을 누른 뒤 `RoomSelection`의 고정 원인 코드와 앱의 제목 확인·등록을 확인합니다. 선택 세션 도중 다른 접근성 서비스를 억제하는 UiAutomation 덤프는 사용하지 않습니다.
...
`Conversation storage tests`의 `device-test-apks` 산출물에는 같은 CI 작업에서 빌드한 debug 앱과 instrumentation APK가 포함됩니다. 실기기 재검증 시 두 APK를 함께 설치해야 하며, 기존 설치 앱과 서명이 다르면 데이터를 보존한 채 중단합니다. 이 산출물은 에뮬레이터에서 SQLite 테스트를 통과한 검증용 APK이며, ARM64 모델 추론 통과나 배포용 서명을 의미하지 않습니다.
...
- 설정 안내처럼 런타임에 문구가 바뀔 수 있는 요소의 존재 검사는 리소스 ID를 사용합니다. `settings.yaml`은 `text_provider_summary`와 `text_grounding_summary`를 검사하며, 정확한 문구 자체를 검증하는 테스트는 아닙니다.
- 섹션 제목이 보이더라도 바로 아래 항목이 화면 밖에 있을 수 있습니다. `settings.yaml`은 `로컬 응답 엔진`도 별도로 스크롤한 뒤 표시를 검사합니다.
- 설정 저장 후에는 메인 화면의 기존 스크롤 위치가 복원될 수 있으므로 `AI 자동 답장`까지 위로 스크롤한 뒤 복귀를 확인합니다.
