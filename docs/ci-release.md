...
AGP 9.4.0, Gradle 9.7.1, AGP 내장 Kotlin(컴파일러 2.4.20), compile/target SDK 37을 사용합니다. CI JDK는 21, 앱 바이트코드 대상은 11입니다. 의존성은 gradle/libs.versions.toml에 고정하며 LiteRT-LM은 0.17.0입니다. org.json은 JVM testImplementation에만 포함합니다. AGP 기본 Kotlin 컴파일러는 의존성의 2.4 메타데이터를 읽지 못하므로 루트 buildscript classpath에 호환 버전을 지정합니다.
