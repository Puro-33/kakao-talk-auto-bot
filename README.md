...
1. 앱 설치 후 알림 접근 권한을 허용합니다. 권한 허용만으로 새 방의 메시지 본문을 저장하지 않습니다.
2. 메인 화면의 `대화 수집·말투 분석`에서 Android 카카오톡의 텍스트 내보내기 파일을 선택합니다. UTF-8, 최대 10 MiB를 지원합니다.
3. 참여자 목록에서 본인을 선택하고 저장할 방을 직접 선택합니다. 동명이인을 구분할 수 없거나 이전·외부 자동답장이 섞였다면 파일을 정리한 뒤 가져오세요.
4. 말투 미리보기에서 표본 수·분석 기간·갱신 시각을 확인합니다. 최근 90일 중 분석 가능한 발화가 12개 이상이면 적용하며, 방별 본인 표본이 부족하면 전체 본인 말투를 참고합니다.
5. 실제 알림을 받은 방을 직접 연결한 뒤 `이 방의 알림 수집`을 켭니다. 연결은 이후 알림에만 적용하며 기존 기록을 합치지 않습니다.
6. `AI 답장 설정`에서 모델을 준비하고, `대상 방 관리`에서 응답할 방과 조건을 설정한 뒤 `AI 자동 답장`을 켭니다. 직접 적는 예문·말투는 선택 사항입니다.
