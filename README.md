...
2. 선택을 시작한 뒤 2분 안에 카카오톡 채팅 목록에서 원하는 방을 직접 누르고 앱으로 돌아와 방 이름을 확인합니다. 방 선택에는 기존 알림이나 대화 파일이 필요하지 않습니다. **이름이 같은 방은 구분할 수 없으므로**, 사용 전에 카카오톡 방 이름을 다르게 설정해야 합니다.
...
사용자가 카카오톡 채팅 목록에서 직접 누른 방 이름을 접근성 서비스로 읽고, 앱에서 확인한 뒤 등록합니다. 선택은 2분 동안만 유효하며 메시지 내용 수집이나 자동 클릭은 하지 않습니다. 저장한 ID는 이 앱의 로컬 ID이며 카카오톡의 고유 채팅방 ID가 아닙니다. 동명 방은 구분할 수 없습니다. 이후 대화 수집은 수신 알림을 사용하고 과거 대화 파일 가져오기는 별도 선택 사항입니다. [구현 범위와 공식 API 조건](docs/kakao-room-selection.md)을 참고하세요.
